...
| Get cleanup policies applied on asset | GET    | `/projects/{assetId}/policies`                         | Gets policies applied on a specific asset.          \<br /\> The API currently only returns direct policies applied on asset (which is needed for MV). API response returns a list to handle multiple policies in future, however, response will be enhanced to indicate direct vs inherited policy. |
